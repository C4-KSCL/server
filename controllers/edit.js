exports.editInfo = (req, res) => {
    ...
    const { password, nickname, phoneNumber, age} = req.body;
    // 유저 정보 업데이트문을 수행하는 쿼리문
    const updateQuery = `UPDATE User SET password = ?,nickname = ?,phoneNumber = ?,age = ? WHERE email = ?`;
    req.mysqlConnection.query(updateQuery, [password, nickname, phoneNumber, age, email], (err, results) => {
        if (err) {
...
exports.editInfoMBTI = (req, res) => {
    const email = req.headers.email;
    if (!email) {
        return res.status(300).send('이메일 헤더가 제공되지 않았습니다.');
    }
    const { myMBTI} = req.body;
    // 유저 정보 업데이트문을 수행하는 쿼리문
    const updateQuery = `UPDATE User SET myMBTI = ? WHERE email = ?`;
    req.mysqlConnection.query(updateQuery, [myMBTI, email], (err, results) => {
        if (err) {
            console.error('Error while querying:', err);
            return res.status(500).send('서버 에러');
        }
...
exports.editInfoKeyword = (req, res) => {
    const email = req.headers.email;
    if (!email) {
        return res.status(300).send('이메일 헤더가 제공되지 않았습니다.');
    }
    const { myKeyword} = req.body;
    // 유저 정보 업데이트문을 수행하는 쿼리문
    const updateQuery = `UPDATE User SET myKeyword = ? WHERE email = ?`;
    req.mysqlConnection.query(updateQuery, [myKeyword, email], (err, results) => {
        if (err) {
            console.error('Error while querying:', err);
            return res.status(500).send('서버 에러');
        }
