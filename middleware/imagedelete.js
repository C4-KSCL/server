...
exports.imagedelete = (req, res) => {
    ...
    // 데이터베이스에서 이미지 정보 조회
    const findKeyQuery = 'SELECT imageKey FROM UserImage WHERE imagePath = ?';
    req.mysqlConnection.query(findKeyQuery, [deletePath], (error, results) => {
        ...
        s3.deleteObject(s3Params, (err, data) => {
            ...
            // 데이터베이스에서 이미지 정보 삭제
            const deleteQuery = 'DELETE FROM UserImage WHERE imagePath = ?';
            req.mysqlConnection.query(deleteQuery, [deletePath], (error, results) => {
                if (error) {
                    console.error('Error deleting image from database:', error);
